DROP TABLE IF EXISTS route_distance_50km_750__;
CREATE TABLE route_distance_50km_750__ (id_target int, id_building int, length double precision );
SELECT AddGeometryColumn ('public', 'route_distance_50km_750__','geom', 3035, 'multilinestring', 2);

DROP TABLE IF EXISTS route_targets_750__;
CREATE TABLE route_targets_750__ AS
SELECT id_target, geom
FROM topo_targets
WHERE id_target > 500 and id_target <= 750;
...
CREATE TABLE route_node_ids_750__ (target_ int, farm_ int, node_target_ int, node_farm_ int );
    ...
    BEGIN
    	...
    	n_targets := (SELECT array_agg(id_target) FROM route_targets_750__);
    	...
    	LOOP
    		...
    		INSERT INTO "route_node_ids_750__" (target_, farm_, node_target_, node_farm_)
    		...
    		-- Assign the ids to variables
    	 	node_target_ := (SELECT a.node_target_ FROM route_node_ids_750__ AS a WHERE target_ = id_target_ LIMIT 1);
    		id_building_ := (SELECT array_agg(farm_) FROM route_node_ids_750__ WHERE target_ = id_target_);
    		node_building_ := (SELECT array_agg(node_farm_) FROM route_node_ids_750__ WHERE target_ = id_target_);

    		n_farms := (SELECT COUNT (*) FROM route_node_ids_750__  WHERE target_ = id_target_);
    		...
    		INSERT INTO "route_distance_50km_750__" (id_target, id_building, length, geom)
