...
def Step_01_create_Residuals (allocation, residual, plant_costs):

    # _________________________ Residual resources
    sql_residual = """
    {create_table} AS
    SELECT a.*, 0 AS only_manure, COALESCE(live_methane,0) + COALESCE(crop_methane,0) AS methane_total
    FROM {cost} AS a
    ;
    """.format (
...
def Step_03_calculate_Costs (residual, residual_aggr):
    ...
    sql_cost = """
        ...
        manure AS (
            SELECT f.id_aggregate, f.id_target, f.id_building, f.length, f.manure, f.crop_production, f.live_methane, f.crop_methane, f.methane_total, f.only_manure
            FROM (
                SELECT a.*,
        ...
        join_farms AS (
            SELECT
                ...
                COALESCE(a.live_methane,0) + COALESCE(b.crop_methane,0) AS methane_total, b.only_manure
            FROM crop AS b
        ...
        UPDATE {resources} AS b (
            SET manure_used = a.manure_used,
                crop_used = a.crop_used,
                only_manure = a.only_manure,
...
#
        ...
        # Step_01_create_Residuals(allocation, residual, plant_costs)
