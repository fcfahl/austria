...
DROP TABLE IF EXISTS target_750;
CREATE TABLE target_750 AS
SELECT id_target 
FROM topo_targets 
...
DROP TABLE IF EXISTS topo_route_750;
CREATE TABLE topo_route_750 (id_building integer, id_target integer, cost double precision, distance double precision, length double precision);

SELECT AddGeometryColumn ('public', 'topo_route_750','geom', 3035, 'multilinestring', 2);
...
BEGIN
    -- LOOP TARGET
    FOR loop_targets IN
        SELECT id_target FROM target_750
	...
	LOOP
		...
		INSERT INTO topo_route_750 (id_building, id_target, cost, distance, length, geom)
		VALUES (
