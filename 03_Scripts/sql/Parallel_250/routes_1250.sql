...
DROP TABLE IF EXISTS target_1250;
CREATE TABLE target_1250 AS
SELECT id_target 
FROM topo_targets 
where id_target > 1000 and id_target <= 1250;

			-- Extract Routes

DROP TABLE IF EXISTS topo_route_1250;
CREATE TABLE topo_route_1250 (id_building integer, id_target integer, cost double precision, distance double precision, length double precision);

SELECT AddGeometryColumn ('public', 'topo_route_1250','geom', 3035, 'multilinestring', 2);
...
BEGIN
    -- LOOP TARGET
    FOR loop_targets IN
        SELECT id_target FROM target_1250
	...
	LOOP
		...
		INSERT INTO topo_route_1250 (id_building, id_target, cost, distance, length, geom)
		VALUES (
