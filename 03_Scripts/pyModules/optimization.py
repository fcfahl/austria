...
def Step_02_initialize_Plant (location, links, plant_capacity):
    ...
    sql_links = "{0} (id_aggregate int, id_target int, id_building int, plant_capacity int, length double precision, methane_total double precision, cost_total double precision);".format(create_table(links))
...
def Step_04_select_Plant (links, location, residual_aggr, plant_capacity, rank, minimum_value):
    ...
    sql_select = """
    		SELECT a.id_target, a.rank, a.methane_total_aggr, a.cost_total_aggr, {plant_capacity} AS plant_capacity, a.geom
            {sql_from}
...
def Step_05_update_ID_Links (links, location, residual, plant_capacity, minimum_value):
    ...
    sql_links = """
        ...
        selection AS (
            ...
            WHERE methane_aggregated <= {minimum_value}
        )
        INSERT INTO {links} (id_aggregate, id_target, id_building, plant_capacity, length, methane_total, cost_total)
        SELECT d.id_aggregate, c.id_target, c.id_building, d.plant_capacity, d.length, c.methane_total, c.cost_total
        FROM selection AS c
        LEFT JOIN {residual} AS d
...
def Step_05a_update_Plant_Capacity (location, links):
...
def Step_08_map_Route_Plants (map_routes, location, links):
    ...
    for distance in ['500', '750', '1500', '2000']:
        ...
        sql_merge= """
            INSERT INTO {map_routes} (id_aggregate, id_target, id_building, plant_capacity, length, methane_total, cost_total, farms, route)
            SELECT a.*, b.geom AS farms, c.geom AS route
            FROM {links} AS a
            LEFT JOIN {farms} AS b  ON a.id_building = b.id_building
            LEFT JOIN {route} AS c  ON a.id_building = c.id_building AND a.id_target = c.id_target
...
#
        ...
        while n_rank > 0:
            ...
            Step_05a_update_Plant_Capacity (location, links)
