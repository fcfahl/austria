...
DROP TABLE IF EXISTS target_500;
CREATE TABLE target_500 AS
SELECT id_target 
FROM topo_targets 
...
DROP TABLE IF EXISTS topo_route_500;
CREATE TABLE topo_route_500 (id_building integer, id_target integer, cost double precision, distance double precision, length double precision);

SELECT AddGeometryColumn ('public', 'topo_route_500','geom', 3035, 'multilinestring', 2);
...
BEGIN
    -- LOOP TARGET
    FOR loop_targets IN
        SELECT id_target FROM target_500
	...
	LOOP
		...
		INSERT INTO topo_route_500 (id_building, id_target, cost, distance, length, geom)
		VALUES (
