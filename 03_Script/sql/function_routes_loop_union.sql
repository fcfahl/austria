DROP FUNCTION IF EXISTS jrc_get_routes (id_target_ int, distance int);
CREATE OR REPLACE FUNCTION jrc_get_routes(id_target_ int, distance int)
...
WITH
    ...
    farm AS
    (
	SELECT array_agg(node_building2) AS id_node FROM jrc_get_ids_arrays ($1, $2)
    ),
    dijkstra as (
	SELECT
	...
	FROM dijkstra a
	LEFT JOIN (SELECT * FROM jrc_get_ids_arrays ($1, $2)) b
...
select * from jrc_get_routes (1, 10000)